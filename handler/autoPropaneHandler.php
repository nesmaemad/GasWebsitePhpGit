<?php
    ...
    function getCompanyReviews($conn){
        ...
        $sql = "select user.user_name , price , review , rating , time from auto_propane , user where auto_propane.user_id = user.id and"
                . " company_id = ? and auto_propane.province_id = ?  and auto_propane.city_id = ?";
        $stmt     = $conn->prepare($sql);
        $stmt->bind_param("sss", $company_id , $province_id , $city_id);
        $stmt->execute(); 
    ...
    function postReview($conn){
        ...
        $sql = "insert into auto_propane (country_id,province_id,company_id,user_id,price,review,rating,time,city_id)"
               . "values ( ? ,? ,? ,? ,? ,? ,? ,? ,? )";
        $stmt     = $conn->prepare($sql);
        $stmt->bind_param("sssssssss", $_GET['country_id'] , $_GET['province_id'],$_GET['company_id']
                ,$_GET['user_id'],$_GET['price'],$_GET['review'],$_GET['rating'],$time_stamp, $_GET['city_id'] );
        $stmt->execute(); 
        $stmt-> close();
        
        $users_sql = "select email , first_name from user , auto_propane where "
                . "auto_propane.user_id = user.id  and auto_propane.city_id = ? and user.id != ?";
    ...
    function getReviews($conn){
         ...
         $sql = "select company.name , count(auto_propane.company_id), MIN(price), company.id "
                 . " from company , auto_propane , province  where "
                 . "auto_propane.company_id = company.id  and auto_propane.province_id = province.id"
                 . " and province.id = ?"
                 . " and auto_propane.city_id = ?"
                 . " group by company.name";
         
        $price_sql = "select user.user_name , auto_propane.rating , auto_propane.id , auto_propane.review "
                . "from auto_propane , user  where company_id = ? and auto_propane.user_id = user.id and "
            . "auto_propane.province_id = ?  and auto_propane.price = ? and auto_propane.city_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ss", $province_id , $city_id);
            $stmt->execute(); 
            $stmt->store_result(); 
             ...
             //   $review->id            =  $col6;
               ...
               // $review->review        =  $col8;
                if($price_stmt = $conn->prepare($price_sql)){
                    $price_stmt->bind_param("ssss", $col7 ,$province_id , $col3 , $city_id );
                    $price_stmt->execute();
