<?php
    ...
    function postReview($conn){
        ...
        $stmt-> close();
                ...
                $users_sql = "select email , first_name from user , commercial_review where "
                        . "commercial_review.user_id = user.id and commercial_review.city_id = ? and user.id != ?";
        ...
        while($users_stmt->fetch()){
            ...
            http://superiorchoicemarketing.com/Gas/index.php#!/commercial
    ...
    function getReviews($conn){
         ...
         $sql = "select company.name , count(commercial_review.company_id), MIN(commercial_review.price), company.id"
                 ...
                 . " and commercial_review.volume_id = volume.id and province.id = ? and volume.id = ? "
        ...
        $price_sql = "select user.user_name , commercial_review.rating , commercial_review.id , commercial_review.review , commercial_review.rental "
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssss", $province_id, $volume_id , $commercial_category_id , $city_id);
            $stmt->execute(); 
            $stmt->store_result(); 
